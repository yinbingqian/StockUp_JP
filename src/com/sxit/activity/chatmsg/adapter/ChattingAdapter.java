package com.sxit.activity.chatmsg.adapter;

import java.util.List;

import com.sxit.activity.chatmsg.Chat_Img_Display_Activity;
...
public class ChattingAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null || (holder = (ViewHolder) convertView.getTag()).direction != message.getMsg_direction()) {
			...
			holder.lyImageView = (ImageView) convertView.findViewById(R.id.chat_msg_list_item_content_img);
			...
			holder.lyNews.setOnClickListener(new OnClickListener() {
				...
				intent.putExtra("wap_content", message.getContent3_content());
				intent.putExtra("wap_name", message.getContent3_title());
//				intent.putExtra("wap_name", "股涨咨询");
		...
		} else if (message.getMsg_type() == 1) {//图片
			if (message.getContent1() == null) {
				System.out.println(">>>> getByteContent");
			} else {
				holder.lyVoice.setVisibility(View.VISIBLE);
				holder.tvContent.setVisibility(View.GONE);
				holder.lyNews.setVisibility(View.GONE);
//				holder.tvContent.setText("图片：" + message.getContent2().length + "");
				Instance.imageLoader.displayImage(SOAP_UTILS.PIC_YANBAO+message.getContent1(), holder.lyImageView, Instance.new_s_options);

				holder.lyVoice.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						// AudioWrapper.DoSinglePlayer(message.getContent2());
//						Toast.makeText(context, message.getContent1().toString(), Toast.LENGTH_SHORT).show();
						Intent intent = new Intent();
						intent.setClass(context, Chat_Img_Display_Activity.class);
						intent.putExtra("url", SOAP_UTILS.PIC_YANBAO+message.getContent1());
						context.startActivity(intent);
					}
				});
			}
			...
			Instance.imageLoader.displayImage(SOAP_UTILS.PIC_YANBAO+message.getContent3_img(), holder.xwivImg, Instance.new_s_options);
		}
