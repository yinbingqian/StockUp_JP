		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (position != 0) {
					...
					intent.putExtra("id", list.get(position).getAnwserId());
					intent.putExtra("answer", list.get(position).getTitle());
		...
		if (res.getCode().equals(SOAP_UTILS.METHOD.UPDATABESTANSWER)) {
			if (res.getObj() != null) {
				if (res.getObj().toString().equals("true")) {
					...
					finish();
					EventCache.commandActivity.post(new String []{this.getClass().getName(),list.get(adapter.checkAsw()).getAnwserId()});
				} else {
